...
function add() {
    ...
    otd1.innerHTML =
        '<select onChange="changeOtd2()" name="infoName_txt" id="infoName_txt' + (document.getElementById('tab').rows.length - 1) + '">' +
            ...
            '<option value="calculate_category_number">获取各个类目的行业销售额</option>' +
            '<option value="calculate_main_cat">根据销售额抽取出主要类目</option>' +
            '<option value="fenxiao_category_trend">分析自有类目的销售趋势</option>' +
            '<option value="seller_have_up_category">抽取各类目已经上架的自有商家名单</option>' +
            //'<option value="calculate_main_cat_item">分析类目的趋势以及商家的库存</option>' +
        '</select>';
