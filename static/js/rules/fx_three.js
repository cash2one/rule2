...
function add() {
    ...
    otd1.innerHTML =
        '<select onChange="changeOtd2()" name="infoName_txt" id="infoName_txt' + (document.getElementById('tab').rows.length - 1) + '">' +
            ...
            '<option value="select_itemnum">选择需要分析的产品ID</option>' +
            '<option value="calculate_item_up_number">抽取各产品的上架商家数量</option>' +
            '<option value="item_need2up">根据商家数目抽取需要上架的产品</option>' +
            '<option value="seller_need2up">抽取未上架的商家名单</option>' +
