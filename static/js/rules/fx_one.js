...
function add() {
    ...
    otd1.innerHTML =
        '<select onChange="changeOtd2()" name="infoName_txt" id="infoName_txt' + (document.getElementById('tab').rows.length - 1) + '">' +
            ...
            '<option value="select_object_seller">获取类目对应的商家ID以及商品ID</option>' +
            '<option value="forecast_R1_number">预测商品的未来销量</option>' +
            '<option value="final_handle">分析类目的趋势以及商家的库存</option>' +
        '</select>';
